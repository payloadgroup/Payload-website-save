        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {ROOMS.map((room, index) => {
              ...
              const isActive = isProjectStarted(room.name);
              return (
                <motion.div
                  key={room.id}
                  initial={{ opacity: 0, y: 20 }}
                  ...
                  className={`relative bg-payload-surface border ${isActive ? 'border-payload-neon' : room.borderColor} p-4 sm:p-5 rounded-sm transition-all duration-200 ${
                    unlocked 
                ...
                >
                  ...
                  {isActive && unlocked && (
                    <div className="absolute top-2 right-2 flex items-center gap-1 bg-payload-neon/20 border border-payload-neon/50 px-2 py-0.5 rounded-sm z-20">
                      <div className="w-2 h-2 bg-payload-neon rounded-full animate-pulse" />
                      <span className="font-mono text-[9px] text-payload-neon uppercase">Active</span>
                    </div>
                  )}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <room.icon className={`w-6 h-6 sm:w-8 sm:h-8 ${isActive ? 'text-payload-neon' : room.color}`} />
